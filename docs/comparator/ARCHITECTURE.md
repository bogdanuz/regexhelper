...
- В буфере: инлайн-стили на `<span>` (как в прежней таблице для Confluence) — цвет текста и светлый фон; редакторы чаще сохраняют цвет у `span`, чем у `strong`.
