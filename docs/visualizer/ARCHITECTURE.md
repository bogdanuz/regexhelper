...
- **main.js** импортирует `initVisualizer` из **tools/visualizer/app.js** и вызывает при загрузке DOM (вместе с initApp конструктора/конвертера).
...
1. Пользователь вводит regex в textarea (вставка через Ctrl+V/контекстное меню браузера).
...
6. **Вкладки визуализатора:** `app.js` управляет массивом вкладок (максимум 20) и активной вкладкой. При первом успешном построении диаграммы в сессии используется уже созданная стартовая вкладка «Новая диаграмма» (она переименовывается и получает pattern вместо создания дубликата). При следующих визуализациях из поля ввода или при вызове helper‑функции `openInVisualizer(pattern)` создаётся новая вкладка с заголовком по началу regex (обрезка и очистка спецсимволов). Вкладки можно закрывать; глобальный сброс (`resetVisualizerPanel()` или «Сбросить всё» в шапке) приводит визуализатор к состоянию одной пустой вкладки. Справа от строки вкладок всегда видна компактная кнопка «+», которая создаёт новую пустую вкладку. При переключении вкладок диаграмма перерисовывается через regexper под содержимое выбранной вкладки.
...
- Визуализатор — отдельный инструмент; не трогает модули конструктора/конвертера.
