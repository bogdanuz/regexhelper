...
- **index.html**: секция `#visualizer`, внутри `#content` и `.application` (селектор инициализации regexper), форма `#regexp-form`, поле `#regexp-input`; блок **#visualizer-diagram-block** (область диаграммы + панель Скачать/зум/«Раскрыть на весь экран»), контейнер `#regexp-render`, шаблон `#svg-container-base`. Подключены стили visualizer.css, regexper.css, regexper-overrides.css, regexper-reset.css; скрипт **assets/libs/regexper/regexper.js** до main.js.
...
5. **Блок «область диаграммы + панель»:** в **index.html** область диаграммы и нижняя панель (Скачать, зум, «Раскрыть на весь экран») обёрнуты в **#visualizer-diagram-block** с `max-height: calc(100vh - 260px)` и flex-колонкой — панель всегда внизу и видна без прокрутки страницы. **Панель под диаграммой:** одна кнопка «Скачать» с выпадающим меню (SVG/PNG в ряд, is-open); кнопки зума (−, 100%, +) и «Раскрыть на весь экран». Зум при пустой диаграмме — тост. **Зум и пан как в Miro/Figma:** скроллбара нет (`overflow: hidden` у .visualizer-diagram-scroll). Масштаб и сдвиг задаются одним **transform** на обёртке: `translate(panX, panY) scale(zoom/100)` с `transform-origin: 0 0`. **Ctrl + колёсико** — зум к точке под курсором; кнопки −/+ — зум к центру вьюпорта. **Пан** — перетаскивание ЛКМ меняет panX/panY. **Fit to view:** при первой отрисовке вызывается `applyFitToView()` — диаграмма вписывается в поле (масштаб не больше 100%), центрируется; обёртка имеет размер по контенту (min-width/min-height 0), поэтому и маленькие, и большие диаграммы центрируются одинаково. **Экспорт:** SVG и PNG из текущего SVG с DIAGRAM_EXPORT_STYLES; PNG 2× (PNG_EXPORT_SCALE). Имя файла: `formatDiagramFilename()` → DD-MM-YYYY_HH-mm. **Область диаграммы:** до построения — адаптивная высота (35vh–60vh), has-diagram при отображённой диаграмме (75vh–85vh). **Поле ввода:** без max-height, resize: vertical.
6. **Модальное окно на весь экран:** по кнопке «Раскрыть на весь экран» открывается оверлей; в нём клон #regexp-render, тулбар с экспортом SVG/PNG и зумом. **Та же логика, что в основной панели:** один transform на обёртке (translate + scale), overflow: hidden (без скроллбара), при открытии — fit to view (applyModalFitToView), зум к курсору (Ctrl+колёсико), зум к центру (кнопки −/+), пан перетаскиванием. Закрытие — кнопка × или клик по оверлею.
...
| Модуль | Назначение |
|--------|------------|
| **tools/visualizer/app.js** | initVisualizer: сброс панели при загрузке и при «Сбросить» в шапке (resetVisualizerPanel экспортирована), кнопка «Очистить» в шапке, наблюдатели body и #regexp-render, обработка hash секции, форма (submit, Вставить). **Экспорт:** одна кнопка «Скачать» с выпадающим меню (SVG/PNG в ряд, класс is-open). Закрытие меню по клику на document; клик по самой кнопке «Скачать» не закрывает меню (чтобы открытие было стабильным при программном/внешнем клике). **Зум и пан:** один transform на обёртке scroll-wrap: `translate(panX, panY) scale(zoom/100)`; зум к курсору (Ctrl+колёсико) и к центру (кнопки −/+); пан перетаскиванием. **Fit to view:** при появлении SVG вызывается `applyFitToView()` (масштаб вписывает в поле, не больше 100%; центрирование panX/panY). Размеры SVG — `getNaturalSvgSize()` (getBBox/viewBox), DIAGRAM_PADDING 36px. Класс has-diagram на области при отображённой диаграмме. Кнопка «Раскрыть на весь экран». Модальное окно: клон диаграммы, зум/пан/экспорт, `getModalNaturalSvgSize()`. При has-error — один тост за попытку. Экспорт: resetVisualizerPanel, encodeRegexForHash, formatDiagramFilename. |
...
| **tools/visualizer/css/** | visualizer.css (блок **visualizer-diagram-block**: max-height calc(100vh - 260px), flex-колонка; область диаграммы flex: 1, min-height: 0; панель экспорта flex-shrink: 0; **visualizer-diagram-scroll** overflow: hidden; **visualizer-diagram-scroll-wrap** min-width/min-height: 0, width/height: fit-content — размер по контенту для корректного центрирования больших и маленьких диаграмм; индикатор загрузки — три точки по центру; выпадающее меню «Скачать» вправо, кнопка «Раскрыть на весь экран»; padding 36px), regexper-overrides.css (поле диаграммы белое, палитра с контрастом), regexper-reset.css. |
