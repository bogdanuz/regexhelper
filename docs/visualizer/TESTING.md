...
- **Visualizer Fullscreen** — открытие модалки копирует диаграмму; при открытии применяется fit to view (как в основной панели); зум в модалке меняет значение (кнопки −/+ и Ctrl+колёсико — зум к центру/к курсору); область диаграммы в модалке без скроллбара (overflow: hidden); закрытие по кнопке и по клику по оверлею; экспорт SVG из модалки вызывает createObjectURL.
