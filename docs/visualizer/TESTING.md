...
- **Visualizer Tabs & helpers** — создание новой вкладки при визуализации из поля ввода и при вызове `openInVisualizer` из конвертера/редактора, сохранение предыдущих вкладок, корректное переключение и закрытие, ограничение на максимальное число вкладок и поведение кнопки «Сбросить» (остаётся одна пустая вкладка). Тесты также проверяют, что переходы «В визуализатор» не меняют существующие вкладки, а добавляют новые.
