...
| **Тестер** | [docs/tester/ARCHITECTURE.md](tester/ARCHITECTURE.md) | Проверка regex на тестовой строке: Python (emulated), флаги, Worker с таймаутом, подсветка совпадений и реал-тайм подсветка ошибок в поле regex (скобки, \|, \|\|, лишняя }), Match Information. |
| **Ручной редактор** | [docs/EDITOR_PANEL_SPEC.md](EDITOR_PANEL_SPEC.md) | Ручной ввод/правка regex: вставка параметров в курсор, Проверить, Сохранить в историю, В визуализатор/тестер; приём из панели результата и из карточек истории («В редактор» — полная замена). Кнопка «Инвертировать выделенное» — инверсия выделенного фрагмента и вставка в конец через \|. |
